...
export default function EditionProduct() {
   
    const cookieValue = Cookies.get('usuario');
    const editables = JSON.parse(Cookies.get('editablesP') || '[]');
    const [ images, setImages] = useState([]);
    const [nombre, setNombre] = useState(editables.nombre || '');
    const [descripcion, setDescripcion] = useState(editables.descripcion || '')
    const [precio, setPrecio] = useState(editables.precio || '')
    ...
    const   handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(`http://localhost:1080/api/producto-editar/${editables.id}`, {
                method: 'POST',
                body:formData,
            });
    ...
    return <React.Fragment>
        <div className='editar flex'>
            <div className='left relative '>
                <form onSubmit={handleSubmit}>
                  <h2 className='poppins-semibold'>Al terminar toca inicio para ver tus cambios</h2>
                <input
       ...
       type='text'    value={nombre } // Mostrar 'editables.nombre' si 'nombre' es vacío o nulo
       ...
       <br/><br/>
        ...
        placeholder={editables.descripcion} type='text' value={descripcion}  onChange={(e) => setDescripcion(e.target.value)}  />
       ...
       <br/>
        ...
        placeholder={editables.precio} type='number'  value={precio } onChange={(e) => setPrecio(e.target.value)}  />
         <br/><br/>
         <br/><br/>
